...
build_xml() {
    ...
    javac $COMPILE_OPTIONS -cp $BUILD_CP -sourcepath $SOURCES_DIR -d $BUILD_DIR -Xlint:unchecked $SOURCES_DIR/org/catacombae/xml/*.java
    return $?
}
build_xml_apx() {
...
build_dmgextractor() {
    echo "Building org.catacombae.dmgextractor..."
...
build_dmgx_gui() {
    ...
    javac $COMPILE_OPTIONS -cp $BUILD_CP -sourcepath $SOURCES_DIR -d $BUILD_DIR -Xlint:unchecked $SOURCES_DIR/org/catacombae/dmgx/gui/*.java
    return $?
}
buildjar() {
